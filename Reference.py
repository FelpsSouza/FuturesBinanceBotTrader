...
futures_websocket = 'wss://fstream3.binance.com/ws/{}@kline_{}'.format(
    symbol, tframe)
...
# setting leverage
change_leverage = client.futures_change_leverage(
    symbol=TRADE_SYMBOL, leverage=Lev)
...
df_final['slowest_EMA'] = round(
    talib.EMA(df_final['c'], slowest_EMA), round_off)
df_final['macd'], df_final['macdSignal'], df_final['macdHist'] = talib.MACD(
    df_final['c'], fastperiod=macd_fast, slowperiod=macd_slow, signalperiod=macd_signal)
df_final['hlc_ave'] = (df_final['h'].astype(
    float) + df_final['l'].astype(float) + df_final['c'].astype(float)) / 3
df_final['VWAP'] = (df_final['hlc_ave'] * df_final['v'].astype(float)
                    ).cumsum() / df_final['v'].astype(float).cumsum()
df_final['ATR'] = talib.ATR(
    df_final['h'], df_final['l'], df_final['c'], timeperiod=atr_period)
print(df_final)
...
def on_message(ws, message):
    ...
    if candle_closed:
        ...
        macdSignal_3c_ago = round(
            df_final['macdSignal'].iloc[-3], round_off + 1)
        ...
        print("Open: {}".format(open_data), "  |  " "High: {}".format(high_data),
              "  |  " "Low: {}".format(low_data), "  |  " "Close: {}".format(close_data))
        ...
        if symbol_loc != TRADE_SYMBOL:
            ...
            sys.exit(
                'Warning: Symbol Position and Trade Symbol do not match, bot is restarting..')

        # cancels all open orders
        if float(position_amount) == 0:
            cancel_open_orders = client.futures_cancel_all_open_orders(
                symbol=TRADE_SYMBOL)
            ...
            if float(position_amount) == 0:
                ...
                try:
                    buy_limit_order = client.futures_create_order(
                        symbol=TRADE_SYMBOL, side='BUY', type='LIMIT', timeInForce='GTC', price=entry_price, quantity=TRADE_QUANTITY)
                    ...
                    while order_status != 'FILLED':
                        ...
                        order_status = client.futures_get_order(
                            symbol=TRADE_SYMBOL, orderId=order_id)['status']
                        print(order_status)

                        if order_status == 'FILLED':
                            time.sleep(1)
                            set_stop_loss = client.futures_create_order(
                                symbol=TRADE_SYMBOL, side='SELL', type='STOP_MARKET', quantity=TRADE_QUANTITY, stopPrice=stop_loss)
                            time.sleep(1)
                            set_take_profit = client.futures_create_order(
                                symbol=TRADE_SYMBOL, side='SELL', type='TAKE_PROFIT_MARKET', quantity=TRADE_QUANTITY, stopPrice=take_profit)
                            break
                        ...
                        if time.time() > timeout:
                            ...
                            if order_status == 'PARTIALLY_FILLED':
                                cancel_order = client.futures_cancel_order(
                                    symbol=TRADE_SYMBOL, orderId=order_id)
                                time.sleep(1)

                                pos_size = client.futures_position_information()
                                df = pd.DataFrame(pos_size)
                                pos_amount = abs(
                                    float(df.loc[SYMBOL_POS, 'positionAmt']))

                                time.sleep(1)
                                set_stop_loss = client.futures_create_order(
                                    symbol=TRADE_SYMBOL, side='SELL', type='STOP_MARKET', quantity=pos_amount, stopPrice=stop_loss)
                                time.sleep(1)
                                set_take_profit = client.futures_create_order(
                                    symbol=TRADE_SYMBOL, side='SELL', type='TAKE_PROFIT_MARKET', quantity=pos_amount, stopPrice=take_profit)
                                break
        ...
        if float(macd_3c_ago) > float(macdSignal_3c_ago) and float(last_macd) < float(last_macdSignal) and float(macd_2c_ago) > 0 and float(last_close) < float(last_slowest_EMA) and float(last_close) < float(last_VWAP) and SL_range_sell <= stop_range:
            ...
            if float(position_amount) == 0:
                ...
                try:
                    sell_limit_order = client.futures_create_order(
                        symbol=TRADE_SYMBOL, side='SELL', type='LIMIT', timeInForce='GTC', price=entry_price, quantity=TRADE_QUANTITY)
                    order_id = sell_limit_order['orderId']
                    order_status = sell_limit_order['status']

                    timeout = time.time() + (50 * tme_frame)
                    while order_status != 'FILLED':
                        # check every 10sec if limit order has been filled
                        time.sleep(10)
                        order_status = client.futures_get_order(
                            symbol=TRADE_SYMBOL, orderId=order_id)['status']
                        print(order_status)
                        ...
                        if order_status == 'FILLED':
                            time.sleep(1)
                            set_stop_loss = client.futures_create_order(
                                symbol=TRADE_SYMBOL, side='BUY', type='STOP_MARKET', quantity=TRADE_QUANTITY, stopPrice=stop_loss)
                            time.sleep(1)
                            set_take_profit = client.futures_create_order(
                                symbol=TRADE_SYMBOL, side='BUY', type='TAKE_PROFIT_MARKET', quantity=TRADE_QUANTITY, stopPrice=take_profit)
                            break
                        ...
                        if time.time() > timeout:
                            ...
                            if order_status == 'PARTIALLY_FILLED':
                                ...
                                time.sleep(1)
                                set_stop_loss = client.futures_create_order(
                                    symbol=TRADE_SYMBOL, side='BUY', type='STOP_MARKET', quantity=pos_amount, stopPrice=stop_loss)
                                time.sleep(1)
                                set_take_profit = client.futures_create_order(
                                    symbol=TRADE_SYMBOL, side='BUY', type='TAKE_PROFIT_MARKET', quantity=pos_amount, stopPrice=take_profit)
                                break
...
ws = websocket.WebSocketApp(
    futures_websocket, on_open=on_open, on_close=on_close, on_message=on_message)
ws.run_forever()
