...
# Sell
        if (last_RSI > RSI_OB) and (last_MFI > MFI_OB):
            if in_position:
                print("Condições Respeitadas, mas venda ja efetuada")
                ...
                order_succeeded = order(SIDE_SELL, TRADE_QUANTITY, cfg.symbol)
...
# Buy
        ...
        if (last_RSI < RSI_OS) and (last_MFI < MFI_OS):
            if in_position:
                print("Condições respeitadas, mas compra ja efetuada")

            else:
                print("COMPRAR! COMPRAR! COMPRAR!")
                order_succeeded = order(SIDE_BUY, TRADE_QUANTITY, cfg.symbol)

                if order_succeeded:
                    in_position = True

# Verificar se precisa parar uma compra ou venda

        if percent_profit > 2:
            print("Condicao de TAKE PROFIT")
            if lastOrder == 'BUY':
                takeProfit = stop_Order(
                    TRADE_SYMBOL, SIDE_SELL, 'MARKET', TRADE_QUANTITY)
            elif lastOrder == 'SELL':
                takeProfit = stop_Order(
                    TRADE_SYMBOL, SIDE_BUY, 'MARKET', TRADE_QUANTITY)
            pprint.pprint(takeProfit)
            in_position = False

        if percent_profit < -2:
            print("Condicao de STOP LOSS")
